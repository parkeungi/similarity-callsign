  ...
  return {
    ...
    prompt: {
      instruction: [
        ...
        '모든 쌍은 이미 "높음" 또는 "매우높음" 위험도로 분류되어 있으며,',
        '당신의 역할은 같은 위험등급 내에서 어떤 쌍이 더 우선 조치가 필요한지',
        '1~100점으로 세분화하여 우선순위를 매기는 것입니다.',
      ].join('\n'),
      ...
      scoringMethod: {
        ...
        baseScoreByReasonType: {
          SAME_NUMBER: { base: 75, range: '75~100', description: '다른 항공사, 편명번호 완전 동일 (예: ESR887 ↔ KAL887)' },
          PHONETIC_DIGIT: { base: 65, range: '65~95', description: '발음 혼동 숫자 조합 포함 (예: 5↔9, 3↔8, 13↔30). phoneticConfusion 테이블과 대조하여 해당 쌍이 존재하면 반드시 선택' },
          CONTAINMENT: { base: 60, range: '60~90', description: '짧은 번호가 긴 번호에 포함 (예: KAL126 ↔ KAL1256)' },
          TRANSPOSITION: { base: 55, range: '55~85', description: '숫자 자릿수 전치 (예: TWB301 ↔ TWB310)' },
          SIMILAR_CODE: { base: 55, range: '55~85', description: '항공사코드 발음 유사 + 편명 유사 (예: JNA301 ↔ JJA301)' },
          DIGIT_OVERLAP: { base: 45, range: '45~80', description: '같은 항공사, 앞/뒤 숫자 겹침 (예: AAR701 ↔ AAR731)' },
          LOW_RISK: { base: 25, range: '1~45', description: '위 유형 해당 없음' },
        },
        adjustments: [
          { factor: 'error_type 존재 (실제 오류 이력)', points: '+15~25' },
          { factor: 'sub_error 심각도 (subErrorScoring 참조)', points: '+5~15 (중첩 가산)' },
          { factor: 'error_rate 높음 (errorRateCalculation 참조)', points: '+5~10' },
          { factor: 'traffic_level="혼잡"', points: '+5~10' },
          { factor: 'error_type=NULL + traffic_level="혼잡" (미보고 위험)', points: '+5~10' },
          { factor: 'coexistence_level="long"', points: '+3~7' },
          { factor: 'total_occurrences ≥ 10 (반복 검출)', points: '+3~5' },
          { factor: 'direction 동일 (동시 교신 가능)', points: '+2~5' },
          { factor: 'atc_recommendation="즉시조치"', points: '하한 70점 보장' },
          { factor: 'same_airline_code="일치"', points: '+3~5 (관제 지시 뒤바뀜 위험)' },
          { factor: 'same_number_ratio ≥ 0.5', points: '+2~5' },
          { factor: 'traffic_level="여유"', points: '-3~5' },
          { factor: 'total_occurrences ≤ 3 (산발적)', points: '-2~3' },
          { factor: 'coexistence_level="short"', points: '-2~3' },
        ],
        process: '① reason_type 결정 → ② base score 확인 → ③ adjustments 합산 → ④ 범위 내 클램핑 → ⑤ 분포 제약 확인',
      },
      ...
      reasonTypePriority: {
        ...
        priority: [
          ...
          '7. LOW_RISK — 위 어느 것에도 해당하지 않을 때만',
        ],
      ...
      scoreDistribution: {
        ...
        constraints: [
          ...
          '분포 목표: 전체 쌍이 1~100점 범위에 골고루 분산되도록 채점',
        ],
