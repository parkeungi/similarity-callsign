// 시간대별 충돌 패턴 분석 차트 - 히트맵 + 패턴 분류 테이블 + 시간대 바차트
'use client';
...
import { useTimePatternStats, TimePatternItem } from '@/hooks/useTimePatternStats';
...
export function TimePatternChart() {
  ...
  const { data: stats, isLoading, error } = useTimePatternStats({ minCount: 4, dateFrom, dateTo });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 p-4 shadow-sm">
          <div className="text-xs text-gray-500 mb-1">분석 대상 (4건+)</div>
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-1.5">
                        <span className="font-mono font-bold text-blue-700">{item.my_callsign}</span>
                        <span className="text-gray-400">↔</span>
                        <span className={`font-mono font-bold ${item.airline_code === item.other_airline_code ? 'text-blue-700' : 'text-red-600'}`}>
                          {item.other_callsign}
                        </span>
                      </div>
                    </td>
